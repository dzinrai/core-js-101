...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise((resolve, reject) => {
    if (isPositiveAnswer) resolve(isPositiveAnswer);
    else if (isPositiveAnswer === false) resolve(false);
    reject(isPositiveAnswer);
  }).then(
    (answer) => {
      if (answer) return 'Hooray!!! She said "Yes"!';
...
function chainPromises(/* array, action */) {
  ...
  /* return new Promise((resolve, reject) => {
    resolve(array.map((promise) => promise).then((res) => action(res)));
    reject(array.map((promise) => promise).then((res) => action(res)));
  }).then((s) => s.reduce((val1) => action(val1))).catch((err) => err); */
}
